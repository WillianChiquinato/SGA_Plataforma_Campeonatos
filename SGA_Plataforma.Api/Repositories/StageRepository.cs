...
public sealed class StageRepository : IStageRepository 
{
    ...
    private readonly DbSet<Stage> _dbSet;

    public StageRepository(AppDbContext context)
    {
        _context = context;
        _dbSet = _context.Set<Stage>();
    }
    ...
    public async Task<Stage> CreateAsync(Stage entity, CancellationToken cancellationToken = default)
    {
    ...
    public async Task<Stage?> UpdateAsync(int id, Stage entity, CancellationToken cancellationToken = default)
    {
